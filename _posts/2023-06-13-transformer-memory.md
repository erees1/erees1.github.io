...
Before getting to the memory used by the model I want to touch on one small (and interesting) use of persistent memory which is related to pytorch creating a workspace lazily on the first call to a CuBLAS GEMM. From what I [can](https://github.com/pytorch/pytorch/issues/6962) [gather](https://github.com/pytorch/pytorch/commit/02921e79852e8530911ff89a4beaf7e151c0e8d3) these pools are created to store [CuBLAS handles](https://docs.nvidia.com/cuda/cublas/index.html#thread-safety2), and each cpu thread [must have its own](https://docs.nvidia.com/cuda/cublas/index.html#thread-safety) pool/workspace [local to that thread](https://github.com/pytorch/pytorch/blob/03101a227f6639d5a9ad628d1dc300f9f99a8812/aten/src/ATen/cuda/CublasHandlePool.cpp#L90) (because pytorch runs forwards and backwards pass in seperate cpu threads we get two allocations) and they use 8,519,680 bytes, e.g. sum the numbers on [this line](https://github.com/pytorch/pytorch/blob/03101a227f6639d5a9ad628d1dc300f9f99a8812/aten/src/ATen/cuda/CublasHandlePool.cpp#L51). Shout out to David Macleod for tracking down the sources of these allocations in the PyTorch source code!
